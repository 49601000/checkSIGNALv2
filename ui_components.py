...
**RSI**: {tech["rsi"]:.1f} ｜ **BB判定**: {tech["bb_icon"]} {tech["bb_text"]}
    ...
    with tab_q:
        ...
        if roe is not None or roa is not None or equity_ratio is not None:
            ...
            st.markdown(
                f"""
- ROE: **{roe:.1f}%**  *（自己資本利益率 → 株主資本をどれだけ増やしているか）*  
- ROA: **{roa:.1f}%**  *（総資産利益率 → 持っている資産をどれだけ効率よく回しているか）*  
- 自己資本比率: **{equity_ratio:.1f}%**  *（財務の安全性 → どれだけ“自前の資本”で立っているか）*
                """
            )
        else:
...
- **例D：ROE 3% / ROA 1% / 自己資本比率 30%**  
  ...
  → 構造的な競争力やビジネスモデルに疑問が残る水準で、**他に良い選択肢があれば優先度はかなり低い**。
        ...
        st.markdown("---")
        st.caption(
            "Qスコアはこれらの指標を 0〜100 に圧縮した“ざっくり指標”です。"
            "最終的な判断では、必ずセクター特性と上のような組み合わせのニュアンスを加味して、"
            "自分の目で「このビジネスはどんなタイプか？」をイメージして使ってください。"
        )
